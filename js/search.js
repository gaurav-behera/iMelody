...
<div style="width: 120px; text-align: center; justify-content:center">
		<button class="start-song-button">
			<img src="assets/play-icon.svg" width="30px" height="30px"/>
		</button>
	</div>
	<div>
	...
	<div style="width: 100px; text-align:center; justify-content:center;">
		<img class="heart-img" src="assets/heart-empty.svg" width=30px>
	</div>
...
durationSlider.addEventListener("input", function () {
	...
	durationValueElement.textContent = `${minutes}:${
		seconds < 10 ? "0" : ""
	}${seconds}`;
	inputChange(50);
});
		...
		fetch(url)
			...
			.then((data) => {
				...
				if (results.length != 0) {
					...
					SearchSuggestion.style.zIndex = "1";
					// SearchBar.value = results[0].trackCensoredName;
...
function setTrackOnClick() {
	const heart_icons = document.getElementsByClassName("heart-img");
	// console.log(heart_icons);
	for (let index = 0; index < heart_icons.length; index++) {
		heart_icons[index].addEventListener("click", () => {
			if (heart_icons[index].src.includes("empty")) {
				heart_icons[index].src = "assets/heart-full.svg";
			} else if (heart_icons[index].src.includes("full")) {
				heart_icons[index].src = "assets/heart-empty.svg";
			}
		});
	}
	...
	playSong(firstSong);
	audioPlayer.pause();
	document.getElementById("play-pause-icon").src = "assets/play-icon.svg";
	player.style.visibility = "visible";
}

SearchBar.addEventListener("input", () => {
	// if (SearchBar.value != "" && SearchBar.value.length % 2 == 0)
	inputChange(50);
});
...
function playSong(resultItem) {
	...
	document.getElementsByClassName("player-image")[0].src =
		resultItem.getElementsByClassName("song-image")[0].src;
	document.getElementById("play-pause-icon").src = "assets/play-icon.svg";
	...
	audioPlayer.play();
	document.getElementById("play-pause-icon").src = "assets/pause-icon.svg";
}
...
audioPlayer.addEventListener("timeupdate", () => {
	...
	document.getElementById("current-time").textContent = `${minutes}:${
		seconds < 10 ? "0" : ""
	}${seconds}`;
	setInterval(() => {
